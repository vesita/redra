...
pub fn decode_command(buffer: &[u8]) -> Result<Command, String> {
...
/// 自动解码函数
/// 
/// 该函数调用decode_command来解析协议数据，目前只解析一个Command消息
...
/// * `Err(String)` - 解码失败时返回错误信息
pub fn auto_decode_commands(buffer: &[u8]) -> Result<Vec<Command>, String> { 
    // 这个函数现在只解析一个Command消息，因为linker已经处理了trailer
