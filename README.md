...
- **文件管理**: 帧数据持久化（SQLite .db）、.rdra 向后兼容导入、PCD 点云文件加载
- **数据查询**: 按材质、标签文本检索帧数据
...
- **另存为**：将当前数据库导出为 `.db` 文件（SQLite 格式）
- **从文件加载**：打开 `.db`（原生）、`.rdra`（旧版兼容）、`.pcd` 点云文件
- **导入 .rdra**：将旧格式数据追加到当前数据库
- **清空**：清除所有帧数据

数据存储于 SQLite，每帧写入后立即持久化，无需累积内存。
支持按材质名和标签文本查询实体。
