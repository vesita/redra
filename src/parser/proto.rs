...
pub fn process_pack(pack: command::Command, sender: mpsc::Sender<RDPack>) {
    match pack.cmd {
        ...
        Some(command::command::Cmd::Designation(ref designation)) => {
            ...
            if let Some(designation::design_cmd::Cmd::Spawn(spawn)) = &designation.cmd {
                ...
                if let Some(ref shape_oneof) = spawn.shape {
                    match &shape_oneof.shape {
                        Some(shape::shape::Shape::Point(point)) => {
                            // 处理Point数据
                        ...
                        Some(shape::shape::Shape::Segment(segment)) => {
                            // 处理Segment数据
                        ...
                        Some(shape::shape::Shape::Sphere(sphere)) => {
                        ...
                        Some(shape::shape::Shape::Cube(cube)) => {
                            info!("创建SpawnPack");
                            debug!("Cube: {:?}", cube);
        ...
        Some(command::command::Cmd::Translation(ref translation)) => {
            // 处理Translation命令
            info!("处理Translation命令: {:?}", translation);
        }
        None => {
