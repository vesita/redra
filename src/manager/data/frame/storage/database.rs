...
use super::{SerializableKeyFrame, EntityData, TransformData, MeshData};
use crate::manager::data::frame::{KeyFrame, Inpto};
...
impl From<SerializableKeyFrame> for KeyFrame {
    fn from(serializable: SerializableKeyFrame) -> Self {
        ...
        for entity in serializable.entities {
            // 注意：这里需要确保 expto::rdmp::ExMesh 可用且实现了 Default 或有合适的构造方式
            // 如果 Mesh 数据也序列化了，应该从 entity.mesh 恢复
            let inpto = Inpto::new(
                expto::rdmp::ExMesh::default(), // 简化处理，实际应从序列化数据恢复
                entity.material,
...
impl FrameStorage {
    ...
    pub async fn load_keyframe(&self, frame_id: u32) -> Result<KeyFrame, String> {
        ...
        let (_, _, data_path) = record;
        
        // 读取并反序列化
        let data = fs::read(&data_path).await
            .map_err(|e| format!("读取帧数据失败: {}", e))?;
        
        let serializable: SerializableKeyFrame = bincode::deserialize(&data)
            .map_err(|e| format!("反序列化帧数据失败: {}", e))?;
    ...
    pub async fn clear_all(&self) -> Result<(), String> {
        ...
        if self.db.data_dir.exists() {
            let mut entries = fs::read_dir(&self.db.data_dir).await
