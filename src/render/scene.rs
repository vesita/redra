    ...
    match expto::config::load_static_scene_config(config_path) {
        Ok(config) => {
            if config.global.enabled {
                ...
                render_static_entities(&mut commands, &mut meshes, &asset_server, &material_manager, &keyframe);
                log::info!("静态场景加载完成");
            } else {
...
fn spawn_default_axes(
    ...
    helpers::spawn_entity(commands, meshes, asset_server, material_manager,
        ...
        "materials/base/red.toml", "X_Axis_Cylinder");
    helpers::spawn_entity(commands, meshes, asset_server, material_manager,
